...
export class AccountsPrismaRepository implements AccountRepositoryPort {
    ...
    async search(search: SearchParams): Promise<SearchResult> {
        try {
            ...
            const where: Prisma.AccountWhereInput=search.filter? {name: {contains: search.filter,mode: 'insensitive'}}:{}
            const orderBy: Prisma.AccountOrderByWithRelationInput=search.sort&&this.sortableFields.includes(search.sort)? {[search.sort]: search.sortDirection||'asc'}:{createdAt: 'desc'}
